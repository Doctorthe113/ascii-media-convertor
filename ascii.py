...
DENSITY = np.array([" ", ".", ":", "-", "=", "+", "*", "#", "%", "@"], dtype=str)
SHARPNESS_KERNAL = np.array([[-1, -1, -1], [-1, 9, -1], [-1, -1, -1]])
IMG_EXTENSIONS = (".png", ".jpg", ".jpeg")
FONT = pygame.font.Font("./assets/Inconsolata_Expanded-Black.ttf", 9)


def ascii_ify_img(input_path, output_path, out_height, text):
    image = cv2.imread(input_path)
    height, width = image.shape[:2]
    ASPECT_RATIO = width / height
    RESIZE_IMG_SIZE_H = out_height  # 1080 / 10 = 108
    RESIZE_IMG_SIZE_W = np.ceil(RESIZE_IMG_SIZE_H * ASPECT_RATIO).astype(int)

    ASCII_IMG_H = RESIZE_IMG_SIZE_H * 10  # all frames will 1080px in height
    ...
    img = cv2.resize(src=img, dsize=(RESIZE_IMG_SIZE_W, RESIZE_IMG_SIZE_H))
    img = cv2.filter2D(img, -1, SHARPNESS_KERNAL)

    indices = np.floor(0.0353 * img).astype(int)
    asciiArr = DENSITY[indices.ravel()]
    asciiimgArr = np.reshape(
        a=asciiArr, newshape=(RESIZE_IMG_SIZE_H, RESIZE_IMG_SIZE_W)
    )
    ...
    if not text:
        ...
        for asciiLine in asciiimgLines:
            ascii_surface = FONT.render(asciiLine, False, (255, 255, 255), (0, 0, 0))
...
@click.option(
    "--output-height",
    "-h",
    help="Height of the output image/video/text. Default is 108",
    default=108,
)
@click.option(
    "--text/--no-text",
    "-t/-nt",
    help="Generates a text instead of a image. Doesn't work with videos",
    default=False,
)
def ascii_ify(input_path, output_path, codec, output_height, text):
    ...
    if input_path.endswith(IMG_EXTENSIONS):
        ascii_ify_img(input_path, output_path, output_height, text)
        endTime = time.time()
    ...
    ASPECT_RATIO = width / height
    RESIZE_FRM_SIZE_H = output_height  # 1080 / 10 = 108
    RESIZE_FRM_SIZE_W = np.ceil(RESIZE_FRM_SIZE_H * ASPECT_RATIO).astype(int)

    ASCII_FRM_H = RESIZE_FRM_SIZE_H * 10  # all frames will 1080px in height
    ...
    while vid.isOpened():
        ...
        frm = cv2.resize(src=frm, dsize=(RESIZE_FRM_SIZE_W, RESIZE_FRM_SIZE_H))
        frm = cv2.filter2D(src=frm, ddepth=-1, kernel=SHARPNESS_KERNAL)

        indices = np.floor(0.0353 * frm).astype(int)
        asciiArr = DENSITY[indices.ravel()]
        asciiFrmArr = np.reshape(
            a=asciiArr, newshape=(RESIZE_FRM_SIZE_H, RESIZE_FRM_SIZE_W)
        )
